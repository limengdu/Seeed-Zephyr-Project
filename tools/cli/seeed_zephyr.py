...
# Built-in templates for `create --blank` (minimal board-agnostic Zephyr app).
# `create --blank` 使用的内置模板(最小、板级无关的 Zephyr 应用)。
BLANK_CMAKELISTS = (
...
BLANK_PRJ_CONF = "CONFIG_PRINTK=y\n"
BLANK_MAIN_C = (
    "#include <zephyr/kernel.h>\n"
    "#include <zephyr/sys/printk.h>\n\n"
    "int main(void)\n"
    "{\n"
    "\tprintk(\"*** Seeed XIAO Zephyr Base | board: %s ***\\n\", CONFIG_BOARD);\n\n"
    "\twhile (1) {\n"
    "\t\tk_sleep(K_SECONDS(1));\n"
    "\t}\n\n"
    "\treturn 0;\n"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    create.add_argument(
        ...
        dest="from_asset",
        default=None,
        help="Source asset, such as xiao_esp32c6/blinky. Omit with --blank.",
    )
    create.add_argument(
        "--blank",
        action="store_true",
        help="Generate a minimal blank Zephyr project instead of copying an example.",
    )
    create.add_argument(
        "--board",
        dest="board_id",
    ...
    set_pins = subparsers.add_parser(
        "set-pins", help="Write Grove pin assignments into a generated project."
    )
    set_pins.add_argument("board_id", help="Board id such as xiao_esp32c6.")
    set_pins.add_argument(
        "--app",
        required=True,
        help="Path to a generated Grove Zephyr application directory.",
    )
    set_pins.add_argument(
        "--pin",
        dest="pins",
        ...
        default=None,
        metavar="Dn|role=Dn",
        help="Grove pin assignment, e.g. --pin D2 or --pin signal=D2.",
    )
    add_json_flag(set_pins)
    set_pins.set_defaults(func=cmd_set_pins)
...
def run_monitor(board_id: str, port: str | None = None, baud: int = 115200) -> None:
    ...
    # Non-Espressif boards use the reconnecting monitor from the Zephyr venv.
    # 非 Espressif 开发板使用 Zephyr venv 中的可重连监视器。
    port = wait_for_serial_port_ready(port, baud)
    python = zephyr_venv_python()
    print(f"Opening serial monitor: {port} @ {baud} baud", flush=True)
    print("Press Ctrl+] to exit. The monitor keeps waiting if the device drops.", flush=True)
    run_command([str(python), str(SERIAL_MONITOR_SCRIPT), port, str(baud)])
...
def _strip_baked_pin_block(text: str) -> str:
    # Removes a previously baked pin block so re-baking never stacks duplicates.
    # 移除此前烘焙的引脚块，使重复烘焙不会叠加重复内容。
    begin = text.find(_PIN_BLOCK_BEGIN)
    if begin == -1:
        return text
    end = text.find(_PIN_BLOCK_END, begin)
...
def load_app_grove_example(app_path: str) -> dict[str, object]:
    # Loads Grove example metadata from a generated standalone project.
    # 从生成后的独立项目读取 Grove example metadata。
    app_dir = Path(app_path).expanduser().resolve()
    if not app_dir.is_dir():
        raise CliError(f"Application directory not found: {app_dir}")
    example_file = app_dir / "example.yaml"
    if not example_file.is_file():
        raise CliError(f"Generated Grove project metadata not found: {example_file}")
    example = read_structured_yaml(example_file)
    if example.get("kind") != "grove":
        raise CliError("Pin configuration is available for Grove projects only.")
    example["path"] = str(app_dir)
    return example


def update_snapshot_pins(app_dir: Path, board_id: str, assignments: dict[str, str]) -> None:
    # Preserves existing snapshot metadata while recording the latest pin assignment.
    # 保留已有 snapshot metadata，并记录最新引脚分配。
    snapshot_file = app_dir / "snapshot.json"
    ...
    if snapshot_file.is_file():
        try:
            ...
            raise CliError(f"snapshot.json is not valid JSON: {error}") from error
    snapshot.setdefault("generator", "seeed-zephyr")
    snapshot["board"] = board_id
    snapshot["pins"] = assignments
    snapshot_file.write_text(json.dumps(snapshot, indent=2) + "\n", encoding="utf-8")


def cmd_create(args: argparse.Namespace) -> None:
    # Creates a standalone project. --blank scaffolds a minimal app; otherwise a
    # repository example is copied. Board examples copy as-is; Grove examples are
    # board-agnostic and may be generated for any supported board, with --pin baked
    # into a per-board overlay.
    # 创建独立项目。--blank 生成最小应用;否则复制仓库示例。板级示例原样复制;
    # Grove 示例可跨板生成,--pin 固化进按板 overlay。
    if getattr(args, "blank", False):
        if args.from_asset:
            raise CliError("Use either --from <asset> or --blank, not both.")
        _cmd_create_blank(args)
        return
    if not args.from_asset:
        raise CliError("Provide --from <asset> or --blank.")
...
def _cmd_create_blank(args: argparse.Namespace) -> None:
    # Scaffolds a minimal, board-agnostic Zephyr application from built-in templates.
    # 用内置模板生成一个最小、板级无关的 Zephyr 应用。
    if args.pins:
    ...
    project_name = out_dir.name or "app"

    (out_dir / "src").mkdir(parents=True, exist_ok=True)
    (out_dir / "CMakeLists.txt").write_text(BLANK_CMAKELISTS.format(name=project_name), encoding="utf-8")
    (out_dir / "prj.conf").write_text(BLANK_PRJ_CONF, encoding="utf-8")
    (out_dir / "src" / "main.c").write_text(BLANK_MAIN_C, encoding="utf-8")
    ...
    print(f"Created blank project at {out_dir} for {board['id']}", flush=True)
...
def _cmd_create_grove(args: argparse.Namespace) -> None:
    ...
    if assignments:
        bake_pin_overlay(src_dir, out_dir, args.board_id, assignments)
...
def cmd_set_pins(args: argparse.Namespace) -> None:
    # Writes selected Grove pin assignments into an existing generated project.
    # 将选中的 Grove 引脚分配写入已有生成项目。
    require_board(args.board_id)
    example = load_app_grove_example(args.app)
    if not grove_supports_board(example, args.board_id):
        raise CliError(
            f"Grove example {example.get('module_id', '?')}/{example.get('demo', '?')} "
            f"excludes board '{args.board_id}'."
        )
    if example.get("pin_policy") != "selectable":
        raise CliError(
            f"This example uses pin_policy={example.get('pin_policy')}; pin configuration is read-only."
        )
    assignments = resolve_pin_assignment(example, args.board_id, args.pins)
    ...
    payload = {
        "board": args.board_id,
        "pins": assignments,
        "overlay": overlay.relative_to(app_dir).as_posix(),
    }
    if getattr(args, "as_json", False):
        print(json.dumps(payload, indent=2))
        return
    print(f"Updated {payload['overlay']}", flush=True)
    for role, pin in assignments.items():
        print(f"  {role}: {pin}", flush=True)
...
def cmd_monitor(args: argparse.Namespace) -> None:
    ...
    print("Press Ctrl+] to exit. The monitor keeps waiting if the device drops.\n", flush=True)
    run_command([str(python), str(SERIAL_MONITOR_SCRIPT), port, str(baud)])
