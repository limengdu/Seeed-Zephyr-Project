...
MG24 的 PyOCD CMSIS pack、RA4M1 的 Renesas Flash Programmer CLI。不带 `--board`
运行时，setup 会执行当前仓库覆盖范围内的全量板级依赖安装。
...
| XIAO RA4M1 | `xiao_ra4m1` | `examples/boards/xiao_ra4m1/blinky` | PASS | 可构建；默认使用 Zephyr RFP runner |
...
关于 XIAO RA4M1 RFP 烧录和 USB bootloader 进入方式，见
