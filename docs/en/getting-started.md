...
| XIAO nRF52840 | `xiao_ble` | `examples/boards/xiao_nrf52840/blinky` | PASS | Builds; CLI uses UF2 runner |
...
For XIAO RP2040 or XIAO RP2350 running a repository example that supports
automatic UF2 entry, the CLI normally requests UF2 mode through USB CDC at 1200
baud. If the current firmware does not support that request, or no USB CDC
serial port is visible, hold BOOTSEL while plugging in USB, or hold BOOTSEL and
press RESET. The UF2 mass-storage volume must be visible to the environment
running `west flash`.

XIAO nRF52840 uses the Adafruit nRF52 Bootloader. If no UF2 storage volume is
visible, double-tap `RESET`, wait for the storage volume, then rerun the command.
The repository CLI uses Zephyr's `uf2` runner, so normal UF2 flashing does not
require installing `nrfutil`.

One-sentence summary: UF2 boards still flash by copying to a bootloader storage
drive; nRF52840 enters that mode with a double-tap on RESET.
