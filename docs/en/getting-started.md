...
| XIAO RP2040 | `xiao_rp2040` | `examples/boards/xiao_rp2040/blinky` | PASS | Hardware-tested; UF2 mode needed for each flash |
...
Put the RP2040 or RP2350 board into UF2 mode, then rerun the flash command:

```sh
seeed-zephyr flash xiao_rp2040 --monitor
```

Hold BOOTSEL while plugging in USB, or hold BOOTSEL and press RESET. The UF2
mass-storage volume must be visible to the environment running `west flash`.

One-sentence summary: UF2 boards must expose their bootloader storage drive
before Zephyr can copy the firmware.
