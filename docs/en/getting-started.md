...
For RP2040 or RP2350 boards, make sure a UF2 mass-storage volume is visible,
...
For XIAO RP2040 running this repository firmware, the CLI normally requests UF2
mode automatically through USB CDC at 1200 baud. If the board is running older
firmware or no USB CDC serial port is visible, hold BOOTSEL while plugging in
USB, or hold BOOTSEL and press RESET. The UF2 mass-storage volume must be
visible to the environment running `west flash`.

One-sentence summary: UF2 boards still flash by copying to a bootloader storage
drive; XIAO RP2040 can enter that mode automatically after the repository
